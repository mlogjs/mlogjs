	...
	constructor(scope: IScope, name: string) {
		super(scope, {
                ...
                if (prop instanceof LiteralValue && typeof prop.data === "string") {
                    obj = new ObjectValue(scope, {
                        $eval: new MacroFunction(scope, () => {
                            return this.get(scope, new LiteralValue(scope, "sensor"))[0].call(scope, [prop])
                        }),
                        "$=": new MacroFunction(scope, (value: IValue) => {
                            return this.get(scope, new LiteralValue(scope, "control"))[0].call(scope, [prop, value])
                        })
                    })
                } else {
                    obj = new ObjectValue(scope, {
                        $eval: new MacroFunction(scope, () => {
                            return this.get(scope, new LiteralValue(scope, "read"))[0].call(scope, [prop])
                        }),
                        "$=": new MacroFunction(scope, (value: IValue) => {
                            return this.get(scope, new LiteralValue(scope, "write"))[0].call(scope, [prop, value])
                        })
                    })
                }
                return [obj,[]]
			...
			control: new MacroFunction(scope, (attr: IValue, ...args: IValue[]) => {
				...
				return [null, [new InstructionBase("control", attr.data as string, name, ...args)]];
			}),
