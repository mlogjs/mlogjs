...
export class BaseValue extends VoidValue implements TValue {
	...
	"u-"(scope: IScope): TValueInstructions {
		...
		return [
			temp,
			[...inst, new OperationInstruction("sub", temp, new LiteralValue(scope, 0), that)],
		];
	}
}
...
for (const key in operatorMap) {
	...
	BaseValue.prototype[key] = function (
		...
		return [
			temp,
			[...leftInst, ...rightInst, new OperationInstruction(kind, temp, left, right)],
		];
	};
}
...
for (const key in unaryOperatorMap) {
	...
	BaseValue.prototype[key] = function (this: BaseValue, scope: IScope): TValueInstructions {
		const [that, inst] = this.eval(scope);
		const temp = new TempValue(scope);
...
const assignmentToBinary: { [k in AssignementOperator]?: BinaryOperator|LogicalOperator } = {
	"+=": "+",
	"-=": "-",
	"*=": "*",
	"/=": "/",
...
for (const key of updateOperators) {
	BaseValue.prototype[key] = function (
		...
		const kind = key === "++" ? "+=" : "-=";
		return [ret, [...inst, ...this[kind](scope, new LiteralValue(scope, 1))[1]]];
	};
}